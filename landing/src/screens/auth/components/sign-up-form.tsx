...
import OAuthSignInOptions from "@/screens/auth/components/o-auth-sign-in-options";
import {
  InputOTP,
  InputOTPGroup,
  InputOTPSlot,
...
import { sendEmailOtp, verifyEmailOtp, socialSignIn } from "@/screens/auth/services/auth";
...
const SignUpForm = () => {
  ...
  const handleSendOtp = async (data: SignUpFormData) => {
    setIsPending(true);
    try {
      if (activeTab === "email") {
        const res = await sendEmailOtp(data.email, "sign-up");
        if (res.success) {
      ...
      <div className="transition-all duration-300 ease-in-out">
        {!otpSent ? (
          <form onSubmit={handleSubmit(handleSendOtp)} className="space-y-6">
            {/* Name field is always required for registration */}
            <div>
              <label
                htmlFor="name"
          ...
          <form onSubmit={handleSubmit(handleVerifyOtp)} className="space-y-6">
            <div>
              ...
              <p className="text-xs text-gray-500 mb-4">
                We sent a 6-digit code for registering{" "}
                <span className="font-semibold text-gray-700">{currentName}</span> at{" "}
            ...
            <div className="text-center">
              {resendTimer > 0 ? (
                ...
                <button
                  type="button"
                  onClick={() => handleSendOtp({ name: currentName, email: currentEmail, phone: currentPhone, countryIso2: selectedCountryIso2, otp: "" })}
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              >
                {isPending ? "Verifying..." : "Verify & Sign up"}
              </Button>
            </div>
          </form>
        )}
